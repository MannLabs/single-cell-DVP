...
d %>%
  ...
  filter(Symbol %in% proteome_top_10) %>%
  spread(Symbol, int_core) -> d_top_10

# Plotting expression ratios against true distance
meta_distances_bin %>%
  left_join(d_top_10) %>%
  gather(Symbol, int, (ncol(.)-length(proteome_top_10)+1):ncol(.)) %>%
  ...
  summarise(ratio_gp = median(ratio), sd_gp = sd(ratio)) -> meta_distances_bin_summary
...
meta_distances_bin_summary %>%
  ...
  theme_classic() -> plot_expression_top10
