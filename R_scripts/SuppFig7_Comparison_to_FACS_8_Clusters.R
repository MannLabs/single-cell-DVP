...
DVP  <- data.table::fread(file = paste0("../output/tables/Proteome_to_FACS_8spatialbins.tsv"),             
                                    sep = "\t",
                                    #nrows = 5000,
                                    header = TRUE,
...
FACS <- data.table::fread(file = paste0("../data/external/Moshe_S3_FACS_Proteomics.csv"),
                            sep = ",",
                            #nrows = 5000,
                            header = TRUE,
                            stringsAsFactors = FALSE,
...
FACS <- FACS %>%
 dplyr::select(ends_with("LFQ")|c("Gene names", "Protein IDs"))
...
##########################################################
##### Supplementary Figure Ben-Moshe FACS, panel A ######
#########################################################
...
ggsave(file = paste0("../output/Figures/FACS_comparison__FACS_Line_plot_marker_proteins_normalized_dataset.pdf"),
       width = 7,
       height = 6)
...
ggsave(file = paste0("../output/Figures/FACS_comparison__DVP_Line_plot_marker_proteins_normalized_dataset.pdf"),
       width = 7,
       height = 6)
...
ggsave(file = paste0("../output/Figures/FACS_comparison__Line_plot_marker_proteins_normalized_combined.pdf"),
       width = 7,
       height = 5)
...
# Generate extra dataframe with targets of interest
...
ggsave(file = paste0("../output/Figures/ FACS_comparison__Correlation_Cluster_all_after_matching_correction.pdf"),
       width = 7,
       height = 6)
...
ggsave(file = paste0("../output/Figures/FACS_comparison__Correlation_Cluster_all_after_matching_correction_Outliers.pdf"),
       width = 7,
       height = 6)
...
ggsave(file = paste0("../output/Figures/FACS_comparison__Pearson_Correlation_Testing_Uplot.pdf"),
       width = 6,
       height = 6)
...
write.csv(uplot.df, file = paste0("../output/Tables/FACS_comparison__Comparison_FACS_UPlot_Enrichment_", current_dataset, ".csv"))
...
try({
  ...
  write.csv(webgestalt_out, file = paste0("../output/Tables/Comparison_FACS_GSEA_Enrichment_", current_dataset, ".csv"))
...
pdf(file = paste0("../output/Figures/FACS_comparison__HeatMap_Positive_Correlation_", current_dataset,"_", int.term, ".pdf"), width = 7, height = 5)
