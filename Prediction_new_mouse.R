...
load("./Output/d_classes.R")
load("./Output/d_full_set.R")

## Read image meta data
img_meta <- read_csv("./metaImage/meta_img-proteome.csv") %>%
  mutate(cell_ID = str_replace(cell_ID, "DimethNter", "target"))

img_fluovalues <- read_csv("Z:/Florian_Schober/scDVP/Microscopy/bounding-box/export/2022-09-15_corrected.csv") %>%
  dplyr::rename(bio_ID = Slide) %>%
  left_join(img_meta) %>%
  drop_na(cell_ID)

## Read common contaminats
contaminants <- read_tsv("./contaminants/221109_common-contaminants_cRAP.tsv")
...
# Prediction data
prediction_m1A <- read_prediction("./Predictions/2022-11-23_shape_probability_m1A.csv", bio_ID = "m1A")
